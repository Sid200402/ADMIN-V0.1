...
export default function Brands() {
  ...
  const listQuery = useQuery({
    queryKey: ["brands", page, keyword, status],
    queryFn: () => brandService.getAll({ limit, offset: (page - 1) * limit, keyword, status })
      .then(r => r.data),
    staleTime: 30_000,
  });
  ...
  const statsQuery = useQuery({
    ...
    queryFn: () => brandService.getStats().then(r => r.data?.data || r.data),
    staleTime: 60_000,
  });

  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["brands"] });
    qc.invalidateQueries({ queryKey: ["brands-stats"] });
  };

  // ── Mutations ─────────────────────────────────────────────────────────────
  const createMutation = useMutation({
    mutationFn: (v: any) => brandService.create(v),
    onSuccess: (res) => { const id = res.data?.id || res.data?.data?.id; message.success("Brand created"); setShowForm(false); form.resetFields(); invalidate(); if (id) setFocusedId(id); },
    onError: () => message.error("Failed to create"),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, ...v }: any) => brandService.update(id, v),
    onSuccess: (_, vars) => { const id = vars.id; message.success("Brand updated"); setShowForm(false); setEditItem(null); form.resetFields(); invalidate(); if (id) setFocusedId(id); },
    onError: () => message.error("Failed to update"),
  });

  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: "ACTIVE" | "DEACTIVE" }) => brandService.updateStatus(id, status),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { message.success("Brand deleted"); invalidate(); },
    ...
    },
    {
      ...
      render: (_: any, r: any) => (
        <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
          ...
          <Popconfirm title="Delete this brand?" onConfirm={() => deleteMutation.mutate(r.id)} okText="Yes" cancelText="No">
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: "#111827" }}>Brands</div>
          <div style={{ fontSize: 13, color: "#9ca3af" }}>Manage all brands</div>
        </div>
        <Button type="primary" icon={<PlusOutlined />} onClick={openCreate} style={{ background: "#2563eb" }}>
          New Brand <kbd style={{ background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.35)", borderRadius: 4, padding: "0 5px", fontSize: 10, fontFamily: "monospace", marginLeft: 6 }}>Alt+N</kbd>
        </Button>
      </div>
      ...
      {listQuery.isError && <Alert type="error" message="Failed to load brands." style={{ marginBottom: 12 }} />}
      ...
      <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
        <Table
          ...
          onRow={onRow}
          pagination={{ current: page, pageSize: limit, total, onChange: setPage, showSizeChanger: false, showTotal: t => `Total ${t} brands` }}
        />
      </div>
      ...
      <Modal
        open={showForm}
        title={editItem ? <><EditOutlined /> Edit Brand</> : <><PlusOutlined /> New Brand</>}
        onCancel={closeModal}
        footer={null}
      >
