...
export default function Model() {
  ...
  const listQuery = useQuery({
    queryKey: ["models", page, keyword, status],
    queryFn: () => modelService.getAll({ limit, offset: (page - 1) * limit, keyword, status })
      .then(r => r.data),
    staleTime: 30_000,
  });
  ...
  const statsQuery = useQuery({
    ...
    queryFn: () => modelService.getStats().then(r => r.data?.data || r.data),
    staleTime: 60_000,
  });

  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["models"] });
    qc.invalidateQueries({ queryKey: ["models-stats"] });
  };

  // ── Mutations ─────────────────────────────────────────────────────────────
  const createMutation = useMutation({
    mutationFn: (v: any) => modelService.create(v),
    onSuccess: (res) => { const id = res.data?.id || res.data?.data?.id; message.success("Model created"); setShowForm(false); form.resetFields(); invalidate(); if (id) setFocusedId(id); },
    onError: () => message.error("Failed to create"),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, ...v }: any) => modelService.update(id, v),
    onSuccess: (_, vars) => { const id = vars.id; message.success("Model updated"); setShowForm(false); setEditItem(null); form.resetFields(); invalidate(); if (id) setFocusedId(id); },
    onError: () => message.error("Failed to update"),
  });

  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: "ACTIVE" | "DEACTIVE" }) => modelService.updateStatus(id, status),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { message.success("Model deleted"); invalidate(); },
    ...
    },
    {
      ...
      render: (_: any, r: any) => (
        <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
          ...
          <Popconfirm title="Delete this model?" onConfirm={() => deleteMutation.mutate(r.id)} okText="Yes" cancelText="No">
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: "#111827" }}>Models</div>
          <div style={{ fontSize: 13, color: "#9ca3af" }}>Manage all product models</div>
        </div>
      ...
      {listQuery.isError && <Alert type="error" message="Failed to load models." style={{ marginBottom: 12 }} />}
      ...
      <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
        <Table
          ...
          onRow={onRow}
          pagination={{ current: page, pageSize: limit, total, onChange: setPage, showSizeChanger: false, showTotal: t => `Total ${t} models` }}
        />
      </div>
      ...
      <Modal
        open={showForm}
        title={editItem ? <><EditOutlined /> Edit Model</> : <><PlusOutlined /> New Model</>}
        onCancel={closeModal}
        footer={null}
      >
        <Form form={form} layout="vertical" onFinish={onFinish}>
          <Form.Item name="name" label="Name" rules={[{ required: true, message: "Name is required" }]}>
            <Input placeholder="e.g. X13 PRO" />
          </Form.Item>
          <Button
